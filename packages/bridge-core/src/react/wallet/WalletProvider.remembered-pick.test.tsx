...
describe('WalletProvider — remembered wallet pick', () => {
  ...
  it('does NOT resume off an early announcer while the remembered pick is still pending', async () => {
    // The race (Bugbot, PR #44): extensions announce asynchronously, so Phantom can be the
    // LONE discovered provider for a moment — unambiguous by count — while MetaMask (the
    // remembered wallet) is still on its way. Reading the bare global then would flip
    // canResume on PHANTOM's account; a consumer that auto-resumes would enter a session as
    // the wrong wallet, and sessionEntered would block the restore for the rest of the load.
    readWalletPickMock.mockReturnValue('io.metamask');
    ...
    // Phantom announces FIRST and alone.
    act(() => announce(PH_INFO, ph as unknown as EthereumProvider));
    await waitFor(() => expect(result.current.providers).toHaveLength(1));
    await act(async () => {
      await new Promise<void>((resolve) => setTimeout(resolve, 0));
    });

    // Held back: no account attributed, so nothing can auto-resume onto Phantom.
    expect(result.current.canResume).toBe(false);
    expect(result.current.address).toBeNull();
    expect(callsFor(phantomWindow, 'eth_accounts')).toBe(0);

    // MetaMask announces late — the restore resolves and the read proceeds against IT.
    ...
    act(() => result.current.resumeSession());
    await waitFor(() => expect(result.current.address).toBe(MM_ADDR));
    expect(result.current.address).not.toBe(PHANTOM_ADDR);
  });
