...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  // Invalidation token for the restore, bumped ONLY by disconnect(). Deliberately NOT the
  // connect-attempt token: closeLoginModal() bumps that one, and dismissing a modal says
  // nothing about wanting to abandon a restore — so sharing it meant that opening and
  // cancelling the connect modal during a slow read (a cold MV3 worker takes seconds, which
  // is exactly when the user sees a signed-out UI and pokes at Connect) aborted the restore
  // for the whole page load. Ending the session is the only intent that must stop it.
  const restoreAbortRef = useRef(0);
  // Set once an explicit connect() has PINNED a wallet, and deliberately never cleared for
  // the life of the page: a user who picked a wallet OUTRANKS the record, so the restore must
  // never move the module pin back. Without it the restore steals the pin: connect() moves
  // the selection, which is exactly what makes the parked restore read fail its
  // provider-identity check → retryRestore() releases the once-guard and bumps the retry
  // state → the effect re-runs and re-pins the RECORDED wallet, and the session commits
  // against a pin aimed at a wallet it isn't for, so signMessage routes to the wrong one.
  //
  // STICKY, not in-flight-scoped — an earlier version cleared it in connect()'s `finally`
  // and a test proved that too narrow: when the user REJECTS the account prompt, connect()
  // settles BEFORE the parked read resolves, so the flag was already false and the restore
  // re-pinned and entered the recorded wallet's session — i.e. declining to connect to wallet
  // B silently signed you into wallet A. `sessionEntered` cannot substitute either, since a
  // rejected connect never sets it. A ref, not state: the async guards read it inside the
  // window before React re-renders (see the eager-ref note above), where a mirror is stale.
  const connectClaimedPinRef = useRef(false);
  ...
  useEffect(() => {
    if (restoreAttemptedRef.current || sessionEnteredRef.current || connectClaimedPinRef.current)
      return;
    ...
    restoreAttemptedRef.current = true;
    // Snapshot the restore's own abort token (bumped by disconnect only — see there).
    const abortAt = restoreAbortRef.current;
    void (async () => {
      let accounts: string[];
      try {
      ...
      // consumer's auto-derive, i.e. connected-but-keyless with no prompt. The abort token
      // covers an explicit disconnect() landing mid-read.
      if (
        sessionEnteredRef.current ||
        connectClaimedPinRef.current ||
        restoreAbortRef.current !== abortAt
      )
        return;
      ...
      // Resolve the wallet this session will be PINNED to before entering it. A bare-global
      // record upgrades to a pinned one when — and only when — the wallet we just read from
      // IS a discovered provider, compared by provider OBJECT (never "the sole discovered
      // entry": a wallet can announce while a different, non-announcing one owns the global,
      // and recording that rdns would re-point the session at the wrong wallet). Without the
      // upgrade a record made through the no-picker resume stays on the weaker branch forever
      // and pays the announce-window wait on every load.
      const readRdns = getDiscoveredProviders().find((d) => d.provider === provider)?.info.rdns ?? null;
      // Preferring `live.rdns` outright would break the very rule just stated: the record is
      // shared across tabs, so another tab can re-point it at the SAME address through a
      // DIFFERENT wallet, and the address-equality check above waves that through. We would
      // then pin — and later SIGN with — a wallet that did not answer this read. So a stored
      // rdns is only honoured when it identifies the provider that actually answered; when it
      // disagrees, refuse this load rather than guess (the record survives, and the next load
      // re-pins from it properly and reads through that wallet). This has to be decided
      // BEFORE the commits below — refusing afterwards would leave a session entered but
      // unpinned, which is the state the check exists to prevent.
      if (live.rdns !== null && live.rdns !== readRdns) return;
      const rdns = readRdns;
  ...
  const connect = useCallback(
    async (rdnsOrUuid?: string) => {
      ...
      // Claim the pin BEFORE any await, so a pending restore can never re-point the module
      // selection to the recorded wallet — during this attempt or after it (see the ref).
      connectClaimedPinRef.current = true;
  ...
  const disconnect = useCallback(() => {
    ...
    connectAttemptRef.current += 1;
    // …and the restore's own token, which ONLY this path bumps.
    restoreAbortRef.current += 1;
    clearDeviceIdentity();
