...
describe('WalletProvider — Bugbot round 3', () => {
  it('B5: cancelling the connect modal mid-read does NOT kill the restore', async () => {
    // RED when the restore snapshots the CONNECT-attempt token: closeLoginModal() bumps
    // that token, so opening and dismissing the modal aborted the restore while
    // restoreAttemptedRef stayed true — the session was silently lost for the whole page
    // load. And this is exactly the moment the user pokes at Connect: a cold MV3 worker
    // takes seconds, during which the UI still reads signed-out.
    const parked = deferred([MM_ADDR]);
    recordSession(MM_ADDR, 'io.metamask');
  ...
  it('B5b: but disconnect() mid-read still aborts the restore', async () => {
    // The other half of the split: a dedicated abort token must still be bumped by the one
    // intent that DOES mean "stop restoring". Note this case does NOT pin the token — it
    // stays green with the abort check deleted, because disconnect() also clears the record
    // and the resolve-time re-read refuses on that. It pins the BEHAVIOUR (the abort token
    // is defence in depth, not the only refusal); B5 is what pins the token's scope.
    const parked = deferred([MM_ADDR]);
    recordSession(MM_ADDR, 'io.metamask');
...
describe('WalletProvider — Bugbot round 4', () => {
  it('B6: a pending restore does NOT steal the pin from an in-flight connect()', async () => {
    // RED: connect('app.phantom') MOVES the module selection, which is exactly what makes
    // the parked restore read fail its provider-identity check → retryRestore() releases the
    // once-guard and bumps the retry state → the effect re-runs while connect() is still
    // awaiting its account prompt (sessionEntered still false) → it re-pins to the RECORDED
    // wallet. connect() then commits Phantom's address against a module pin aimed at
    // MetaMask, so signMessage would route to the wallet the session ISN'T for.
    const parked = deferred([MM_ADDR]);
    const phantom = makeProvider([OTHER_ADDR]);
    recordSession(MM_ADDR, 'io.metamask');
    ...
    // The user gives up on the slow read and picks the OTHER wallet explicitly.
    const connecting = act(async () => {
      await result.current.connect('app.phantom');
    });
    // The parked read lands mid-connect — the pin has moved, so this drives retryRestore().
    parked.release();
    await connecting;

    // The explicit pick wins, in BOTH the session and the pin the SDK will sign through.
    expect(result.current.address).toBe(OTHER_ADDR);
    expect(result.current.selectedRdns).toBe('app.phantom');
    expect(result.current.getProvider()).toBe(phantom as unknown as EthereumProvider);
    expect(readWalletSession()).toMatchObject({ address: OTHER_ADDR, rdns: 'app.phantom' });
  });

  it('B6b: a REJECTED connect() does not leave the restore holding the session', async () => {
    // B6 alone does NOT pin the connect-in-flight guard — verified by deleting it and
    // watching all 34 tests stay green, because connect()'s pin re-assert carries that case
    // on its own. This is the case where the re-assert CANNOT help: the user rejects the
    // prompt, so connect() throws before reaching it. Without the guard the restore
    // meanwhile re-pins the recorded wallet and enters ITS session — so declining to connect
    // to Phantom silently signs you into MetaMask instead.
    const parked = deferred([MM_ADDR]);
    const handlers: Record<string, ((...a: unknown[]) => void)[]> = {};
    const phantom = {
      request: vi.fn(async ({ method }: { method: string }) => {
    ...
    // The rejection is reported and NO session exists — not the recorded wallet's.
    expect(result.current.address).toBeNull();
    expect(result.current.sessionRestored).toBe(false);
    expect(result.current.error).toMatch(/rejected/i);
    // And the PIN still belongs to the wallet the user picked. This assertion is what pins
    // the effect's ENTRY guard specifically: without it the retry re-runs the effect, which
    // re-pins the recorded wallet — no session, but the next signMessage would route to a
    // wallet the user never chose.
    expect(result.current.selectedRdns).toBe('app.phantom');
  });

  it('B6c: a rejected connect() to the RECORDED wallet does not restore behind it', async () => {
    // Pins the RESOLVE-TIME half of the guard. Here connect() targets the same wallet the
    // record names, so the pin never moves and the parked read passes its provider-identity
    // check — the entry guard is irrelevant because the read is already in flight. Without
    // the resolve-time check the restore commits a session in the very moment the user
    // declined the prompt.
  ...
  it('B7: refuses when another tab re-pointed the record at a different wallet mid-read', async () => {
    // The resolve-time record re-read only compared the ADDRESS, so a same-address re-point
    // to another wallet was waved through and `live.rdns` was then pinned blindly — a session
    // confirmed by wallet A, routed to wallet B. Same account in two extensions is the case
    // no other check can distinguish: the address matches and signer-binding recovers the
    // same signer, so the pin is the only discriminator.
    const parked = deferred([MM_ADDR]);
    const phantom = makeProvider([MM_ADDR]);
    recordSession(MM_ADDR, 'io.metamask');
    ...
    // Another tab enters the SAME account through Phantom and re-stamps the record.
    recordSession(MM_ADDR, 'app.phantom');
    ...
    // No session: the record names a wallet other than the one that answered our read.
    expect(result.current.address).toBeNull();
    expect(result.current.sessionRestored).toBe(false);
    // The record survives — the next load re-pins from it and reads through Phantom.
    expect(readWalletSession()).toMatchObject({ address: MM_ADDR, rdns: 'app.phantom' });
  });
});
