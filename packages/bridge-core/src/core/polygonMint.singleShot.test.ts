...
describe('fetchCctpMessageByTxHash — every failure throws after ONE request', () => {
  ...
  it('a sole undecodable entry with a non-terminal status stays unmatched', async () => {
    fetchMock.mockResolvedValue(res(200, { messages: [entry('0xdeadbeef', 'pending')] }));

    expect((await reject()) as IrisMessageUnavailableError).toBeInstanceOf(
      IrisMessageUnavailableError,
    );
  });

  it('an AMBIGUOUS response never lends a stranger’s failure to us', async () => {
    fetchMock.mockResolvedValue(
      res(200, { messages: [entry('0xdeadbeef', 'failed'), entry(OTHER_MESSAGE)] }),
    );
