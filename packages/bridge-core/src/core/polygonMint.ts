...
// `match` is required — return burns all mint to the shared inbound anonymizer, so hookData is
// the only thing telling them apart, and a missing selector would answer about a sibling burn.
export async function fetchCctpMessageByTxHash(
  burnTxHash: string,
  opts: { sourceDomain: number; match: CctpMessageMatch },
): Promise<AttestationResult> {
  ...
  if (outcome.kind === 'transient') {
    throw new Error(`Iris message read failed for burn ${burnTxHash}: ${outcome.detail}`);
  }
  ...
  if (entry.status !== 'complete' || !entry.message || !entry.attestation) {
    throw new IrisMessageUnavailableError(
      'incomplete',
      `CCTP message for burn ${burnTxHash} is not attested yet (Iris status "${entry.status}").`,
    );
  }
  return { message: entry.message, attestation: entry.attestation };
}
