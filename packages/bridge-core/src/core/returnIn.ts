...
//
// A missing/non-numeric stamp is past the deadline (pre-stamp cursors predate this deploy).
// A future stamp is conservatively live: a wall-clock step backwards after submission is not
// evidence that the relayer's hash is absent, and clearing it could start a second burn.
function burnCouldStillBeExecuting(cursor: InflightReturn, now = Date.now()): boolean {
  ...
  return now <= submittedAt + DEFAULT_BATCH_DEADLINE_MS + PENDING_BURN_DEADLINE_GRACE_MS;
}
