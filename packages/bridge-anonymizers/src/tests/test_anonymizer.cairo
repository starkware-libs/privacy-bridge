...
//! non-pool caller; as the pool it approves the messenger and burns with the right
//! args + forwarding hook, returning an empty span; `destination_domain` is per-tx;
//! and `BurnInitiated` publicly emits `mint_recipient` + amount.
