...
//!
//! Single-transaction flow (the whole return is one proof-authorized pool tx, so the
//! user's derived Starknet account is never the on-chain sender — closes the
//! account ↔ deposit-wallet identity leak):
...
/// receive+mint is folded into `privacy_invoke_with_computation`, so there is no
/// standalone receive entrypoint and no bound-but-unclaimed ledger.
#[starknet::interface]
