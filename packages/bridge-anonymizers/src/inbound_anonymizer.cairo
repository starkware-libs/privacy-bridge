...
pub trait IInboundAnonymizer<TContractState> {
    ...
    /// Pool compute hook: `commitment = poseidon([identity_key, dapp_name, nonce])`.
    /// `identity_key` is supplied by the pool from the authenticated signer.
    fn privacy_compute(
        self: @TContractState, identity_key: felt252, dapp_name: felt252, nonce: felt252,
    ) -> felt252;
    ...
    fn privacy_invoke_with_computation(
        ref self: TContractState, commitment: felt252, note_id: felt252,
    ) -> Span<crate::types::OpenNoteDeposit>;

    /// Read the bound-but-unclaimed balance for a commitment (used by recovery).
    fn claimable_of(self: @TContractState, commitment: felt252) -> u128;
}
...
pub mod InboundAnonymizer {
    ...
    use super::{
        IInboundAnonymizer, IMessageTransmitterV2Dispatcher, IMessageTransmitterV2DispatcherTrait,
        errors,
    };
    ...
    struct Storage {
        ...
        // Bound-but-unclaimed USDC per commitment, and the escrow-accounted total.
        ledger: Map<felt252, u128>,
    ...
    pub impl InboundAnonymizerImpl of IInboundAnonymizer<ContractState> {
        fn receive_and_bind(ref self: ContractState, message: ByteArray, attestation: ByteArray) {
            ...
            let minted: u128 = (after - before).try_into().expect(errors::AMOUNT_OVERFLOW);
            assert(minted.is_non_zero(), errors::NOTHING_MINTED);

            self.ledger.write(commitment, self.ledger.read(commitment) + minted);
            self.accounted_balance.write(self.accounted_balance.read() + minted);
        ...
        fn privacy_compute(
            ...
            poseidon_hash_span([identity_key, dapp_name, nonce].span())
        }

        fn privacy_invoke_with_computation(
            ref self: ContractState, commitment: felt252, note_id: felt252,
            ...
            self.accounted_balance.write(self.accounted_balance.read() - amount);
        ...
        fn claimable_of(self: @ContractState, commitment: felt252) -> u128 {
    ...
    fn parse_hook_commitment(message: @ByteArray) -> felt252 {
        ...
        result.try_into().expect(errors::COMMITMENT_OVERFLOW)
    }
}
