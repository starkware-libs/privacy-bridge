...
//! Anonymizer-under-test is the real `contracts_cairo::Anonymizer`.
...
/// Mirror of the Anonymizer's `ITokenMessengerMinterV2` (frozen param order).
#[starknet::interface]
...
/// Minimal ERC20 mock: only `approve` + `balance_of` (the surface the
/// Anonymizer's `IERC20` dispatcher actually calls).
#[starknet::interface]
...
/// Test-only stand-in for the privacy pool's hardcoded-selector dispatch. The
/// real pool is an external SDK contract (not deployable in snforge), but it
/// always invokes the Anonymizer through the FIXED `privacy_invoke` selector.
/// `invoke_claim` reproduces exactly that: a raw
/// `call_contract_syscall(anonymizer, selector!("privacy_invoke"), calldata)` so
/// a test can prove the RETURN-leg claim is reachable via the same selector the
/// pool uses — without `start_cheat_caller_address`, the caller IS this MockPool.
#[starknet::interface]
pub trait IMockPool<T> {
    /// Drive `Anonymizer::privacy_invoke` via its hardcoded selector with raw
    /// `calldata` (e.g. a serialized `PrivacyAction::Claim`) and return the raw
    /// serialized result span. Panics if the call reverts (so a test's
    /// `SafeDispatcher` over THIS entrypoint sees the revert).
    fn invoke_claim(
        ref self: T, anonymizer: ContractAddress, calldata: Array<felt252>,
    ) -> Span<felt252>;
}
...
pub mod MockPool {
    ...
    #[abi(embed_v0)]
    impl PoolImpl of IMockPool<ContractState> {
        fn invoke_claim(
            ref self: ContractState, anonymizer: ContractAddress, calldata: Array<felt252>,
        ) -> Span<felt252> {
            // Hardcoded-selector dispatch, exactly like the real pool's InvokeExternal.
            starknet::syscalls::call_contract_syscall(
                anonymizer, selector!("privacy_invoke"), calldata.span(),
            )
                .unwrap()
        }
    }
